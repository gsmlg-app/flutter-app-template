...
5. **Screen creation** → `/project-screen`
   ...
   - Use `DmAdaptiveScaffold` (from `duskmoon_ui`) for responsive layout
...
```
1. /project-database     → Settings storage (preferences)
2. /project-bloc         → SettingsBloc (or use DmThemeBloc from duskmoon_theme_bloc/duskmoon_ui for theme)
...
> **External UI packages (pub.dev):** Theme management is provided by `duskmoon_theme_bloc` (re-exported via `duskmoon_ui`). Forms use `duskmoon_form` (also via `duskmoon_ui`).
...
| Theme preference | `DmThemeBloc` (duskmoon_theme_bloc) | (built-in via duskmoon_ui) |
