...
> **Note**: `DmThemeBloc` is a pre-built BLoC provided by `duskmoon_theme_bloc` (re-exported via
> `duskmoon_ui`). It is NOT a custom BLoC that users subclass or create — use it directly from the
> package. It is shown here as a reference for the BLoC pattern used in this project.

`DmThemeBloc` (from `package:duskmoon_ui/duskmoon_ui.dart`) manages app theme state with
automatic SharedPreferences persistence:

```dart
// Import
import 'package:duskmoon_ui/duskmoon_ui.dart';

// Provide it near the root of your widget tree
BlocProvider(
  create: (context) => DmThemeBloc(sharedPreferences),
  child: BlocBuilder<DmThemeBloc, DmThemeState>(
    builder: (context, state) {
      // state.themeName  — active theme name
      // state.themeMode  — ThemeMode (light/dark/system)
      // state.entry      — DmThemeEntry with .light and .dark ThemeData
      return MaterialApp(
...
// Dispatch events
context.read<DmThemeBloc>().add(DmSetTheme('ocean'));
context.read<DmThemeBloc>().add(DmSetThemeMode(ThemeMode.dark));
```
